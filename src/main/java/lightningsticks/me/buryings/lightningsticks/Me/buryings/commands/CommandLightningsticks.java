...
public class CommandLightningsticks implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!(sender instanceof Player)) {
            ...
            // lsgive <player/all>
            if (args.length > 1) {
                new MSG(player, "%player%, Incorrect Usage! Use: /lsgive <player|all>").replace("%player", player.getName()).send(player);
            }
            if (args.length == 0) {
                if (args[0].equalsIgnoreCase(player.getName())) {
                    for (Player target : Bukkit.getPlayerExact()) {
                        ...
                        new MSG(player, "&aYou have given " + player.getName() + " &alightningstick!");
                    }
